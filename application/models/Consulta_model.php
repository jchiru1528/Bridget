 ...
 function buscar_a ($cedula)
	{
		...
		$SQL="SELECT        (select UPPER(a.decrip_parroquia)FROM tbl_parroquias a where a.id_parroquia=parroquia)parro,parroquia , nombre, apellido, 
		                    cedula, cedula,(case sexo when 'M' then 'Masculino' when 'F' then 'Femenino' end) tsexo ,sexo , DATE_FORMAT( fehca_naci, '%d-%m-%Y' ) fehca_naci, 
      ...
      function buscar_b ($cedula)
		{
			...
			$SQL="SELECT    nombre ,apellido  , edad  ,DATE_FORMAT( fechadenac , '%d-%m-%Y' )fechadenac , nacionalidad , 
